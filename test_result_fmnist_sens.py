...
# kfold_results_c = np.array([[74.05502699922859, 73.95645838690324, 74.08502614211022],
#                             [75.15535936227661, 75.03964342347747, 75.06107230103287],
#                             [76.02537179102559, 75.78536836240518, 76.07251532164744]])

# kfold_results_clean = np.array([[92.68878032056227, 92.7316362389646, 92.6673523613611],
#                                 [92.99275703938628, 92.97989971285304, 92.99704281489736],
#                                 [93.17704538636266, 93.19847426391806, 93.17704538636266]])
...
#     # fold = 0
#     save_dir = './data/FMNIST-C/gaussian_noise_clean_fold_nonsens/'
...
#     labels_c = np.load(f'{save_dir}fmnist_test_labels_c_fold_{fold}.npy')
#     labels_clean = np.load(f'{save_dir}fmnist_test_labels_clean_fold_{fold}.npy')
#     labels_onehot_c = np.load(f'{save_dir}fmnist_test_labels_onehot_c_fold_{fold}.npy')
#     labels_onehot_clean = np.load(f'{save_dir}fmnist_test_labels_onehot_clean_fold_{fold}.npy')
...
#     print(f"Path does not exist. Creating: {f'{save_dir}/set_output_no_reward/'}")
#     os.makedirs(f'{save_dir}set_output_no_reward/')
#     # sys.exit()
...
for alpha_i in tqdm(alpha):
    ...
    for fold in range(k_folds):
        ...
        for j in range(len(all_precise_results)):
            for i in range(len(labels)):
                ...
                if all_precise_results[j][i] == labels[i]:
                    ...
                    #####
                    recal_u[j][0] += m_metric[int(all_precise_results[j][i]), labels[i]] 
                    #####
                    # u65
                    # [[alpha_SED], [alpha_L1], [alpha_KLD]]
                    ...
                    ######
                    list_u_temp = []
                    for pre_dic in all_set_predictions[j][i]:
                        list_u_temp.append(m_metric[int(pre_dic), labels[i]])
                    recal_u[j][0] += max(list_u_temp)
                    ######

        recall_u_final_result = recal_u / len(labels)
        ...
        # the average recall new way
    
        rec_SED = recall_u_final_result[0][0]
        rec_L1D = recall_u_final_result[1][0]
        rec_KLD = recall_u_final_result[2][0]
...
save_dir = root_dir + f'/Compact-credal-deep-ensembling-in-multi-class-classification/Visualize_output/{data_name}/gaussian_{train_type}_fold/{model_type}/'
csv_out.to_csv(f'{save_dir}/set_output_no_reward/{test_type}_set_value_prediction_fixed.csv', header=False)
