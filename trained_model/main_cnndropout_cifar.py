...
from data_preprocess.cifar10_c import CIFAR10C, CIFAR10CLEAN
import saved_loss
...
def evaluate(model, val_loader, save_dir, fold, dataset_type,args):
...
def validate(model_fold, val_loader, criterion,args):
    global best_acc
    model_fold.eval()
    losses = AverageMeter()
    top1 = AverageMeter()
...
def main():
    ...
    global best_prec1_noise,best_prec1_clean,device
    ...
    model = model_loader(args).to(device)
    ...
    if args.mode == 'train':
        ...
        for fold in range(k_folds):
            ...
            for epoch in range(1, args.epochs):
                ...
                train_loss, train_acc = train( epoch, model_fold, train_loader, criterion, optimizer,args)

                val_loss_noise, prec1_noise = validate(model_fold,val_loader, criterion,args)
                
                val_loss_clean, prec1_clean = validate( model_fold,val_loader_clean, criterion,args)
        ...
        for fold in range(k_folds):
            ...
            checkpoint_file = f'{args.save_dir}model/bayesian_{args.model_name}_cifar_{train_type}_fold_{fold}.pth'
            ...
            evaluate( model, val_loader_c,args.save_dir ,fold, 'c',args)
            evaluate( model, val_loader_clean, args.save_dir ,fold, 'clean',args)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='PyTorch DROP OUT CIFAR10 Training')
