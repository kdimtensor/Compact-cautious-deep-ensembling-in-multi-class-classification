...
def frost(x, severity=1):
    ...
    filename = ['./data/frost/frost1.png', './data/frost/frost2.png', './data/frost/frost3.png', './data/frost/frost4.jpg', './data/frost/frost5.jpg', './data/frost/frost6.jpg'][idx]
...
folder_path = root + f'data/{data_name}-C/gaussian_noise/'
os.makedirs(folder_path, exist_ok=True)

# transform = transforms.Compose(transforms.Resize((224,224)))
dataset = datasets.ImageFolder(root=root + f'data/{data_name}/')
...
for img, label in tqdm(dataset):
    ...
    data.append(img)       # img is a tensor (C, H, W)
...
# counts = Counter(data)
# print("Unique values and their counts (using Counter):")    
...
# # Stack into tensors
# # data = torch.stack(data)          # shape: [N, C, H, W]
# # targets = torch.tensor(targets)   # shape: [N]

# # Now you have dataset-like attributes
...
# # Only train data
# # data_clean = dset.CIFAR10('./data', train=True)
...
for method_name in d.keys():
    ...
    np.save(root + f'data/{data_name}-C/gaussian_noise/' + d[method_name].__name__ + '_clean_train.npy', np.array(data_clean).astype(np.uint8))
    np.save(root + f'data/{data_name}-C/gaussian_noise/' + d[method_name].__name__ + '_c_train.npy', np.array(data_c).astype(np.uint8))

    # This replaces not appends
    np.save(root + f'./data/{data_name}-C/gaussian_noise/label_train.npy',
            np.array(labels).astype(np.uint8))

# # Only test data
# data = dset.CIFAR10('./data', train=False)
# convert_img = trn.Compose([trn.ToTensor(), trn.ToPILImage()])
...
#     np.save(root + 'data/CIFAR-10-C/gaussian_noise/' + d[method_name].__name__ + '_clean_test.npy', np.array(cifar_clean).astype(np.uint8))
#     np.save(root + 'data/CIFAR-10-C/gaussian_noise/' + d[method_name].__name__ + '_c_test.npy', np.array(cifar_c).astype(np.uint8))

#     # This replaces not appends
#     np.save(root + './data/CIFAR-10-C/gaussian_noise/label_test.npy',
#             np.array(labels).astype(np.uint8))
