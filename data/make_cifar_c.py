...
####
folder_path = root + 'data/CIFAR-10-C/gaussian_noise/'
...
for method_name in d.keys():
    ...
    np.save(root + 'data/CIFAR-10-C/gaussian_noise/' + d[method_name].__name__ + '_clean_train.npy', np.array(cifar_clean).astype(np.uint8))
    np.save(root + 'data/CIFAR-10-C/gaussian_noise/' + d[method_name].__name__ + '_c_train.npy', np.array(cifar_c).astype(np.uint8))

    # This replaces not appends
    np.save(root + './data/CIFAR-10-C/gaussian_noise/label_train.npy',
            np.array(labels).astype(np.uint8))

# Only test data
data_clean = dset.CIFAR10('./data', train=False)
convert_img = trn.Compose([trn.ToTensor(), trn.ToPILImage()])
